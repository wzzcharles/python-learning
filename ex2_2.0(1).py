    ...
    for i in range(45):
        ...
        for j in range(29):
            ...
            gv_500[time, j, i] = g / (2 * omega * sin((10 + dy * j) * pi / 180) * r ** 2) * (
                    ...
                    dx ** 2 * cos(10 + dy * j) ** 2) + (a[1, time, 5, j + 1, i] + a[1, time, 5, j - 1, i] - 2 * a[1, time, 5, j, i]) / (dy ** 2) - (a[1, time, 5, j + 1, i] - a[1, time, 5, j - 1, i]) * tan((10 + dy * j) * pi / 180) / (2 * dy))  # 计算公式
###实测风涡度
